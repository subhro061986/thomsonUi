...
const ManagePublishersScreen = () => {
  ...
  const savePub = async () => {
    ...
    if (existingId === '') {
      ...
      if (about === '' || gst === '' || pin === '' || stateId === 0
        || contactno === '' || countryId === 0 || commission === ''
        || adressline === '' || maxdiscount === '' || contactemail === ''
        || contactperson === '' || spcom1 === '' || spcom2 === ''
        || imageHandler === null || bannerimageHandler === null || name === ''
        || city === '') {
      ...
      if (resp !== undefined) {
        ...
        if (resp.data.statuscode === '0' && resp.data.message === 'Information updated successfully.') {
  ...
  const pub_det = async (id) => {
    ...
    setContactemail(response.data.output.contactemail);
            ...
            <tbody className="text-center">
              {
                publisherList && publisherList.map((data, index) => (
                  <tr className="custom-table-row" key={index}>
                    ...
                    <td className="all_col text-start">{data.contactemail === null || data?.contactemail?.length === 0 ? 'Not Available' : data.contactemail}</td>
